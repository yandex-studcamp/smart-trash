...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the autoencoder spotter.")
    parser.add_argument(
        "--exp_name",
        required=True,
    ...
    parser.add_argument(
        "--config",
        default="src/config/spotter/train_autoencoder_spotter.json",
...
def main() -> None:
    args = parse_args()
    config = load_autoencoder_spotter_config(args.config)
    config.exp_name = args.exp_name

    experiment_paths = make_experiment_paths(args.exp_name)
    save_autoencoder_spotter_config(config, experiment_paths.config_path)

    summary = train_autoencoder_spotter(config=config, experiment_paths=experiment_paths)
